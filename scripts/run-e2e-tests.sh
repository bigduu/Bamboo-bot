...
if curl -s http://localhost:8080/v1/health > /dev/null; then
    ...
    if [ -f "~/workspace/bodhi/target/release/web_service_standalone" ]; then
        ...
        ~/workspace/bodhi/target/release/web_service_standalone --port 8080 --data-dir /tmp/test-data &
    elif [ -f "~/workspace/bodhi/target/debug/web_service_standalone" ]; then
        echo "使用 debug 版本启动后端..."
        ~/workspace/bodhi/target/debug/web_service_standalone --port 8080 --data-dir /tmp/test-data &
    else
        echo "正在编译后端（可能需要几分钟）..."
        cd ~/workspace/bodhi
