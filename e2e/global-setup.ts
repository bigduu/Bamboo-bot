...
async function globalSetup(config: FullConfig) {
  ...
  try {
    ...
    console.error('   cargo run -p e2e-backend --bin web_service_standalone -- --port 9562 --data-dir /tmp/test-data');
    console.error('');
    console.error('Or start it automatically:');
    console.error('   E2E_START_SERVER="cargo run -p e2e-backend --bin web_service_standalone -- --port 9562 --data-dir /tmp/test-data" yarn test:e2e');
    process.exit(1);
  }
